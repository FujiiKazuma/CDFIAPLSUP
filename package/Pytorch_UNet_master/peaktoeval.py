...
def make_paths(root_path, check_num, lap):
    ...
    ps.append(os.path.join(root_path, f"check{check_num}/lap{lap}/pred_to_eval/peak"))
    return ps
...
def plot(img, img2, cellp, check_num, lap, frame, savepath):
    ...
    # save_path_vector = os.path.join(savepath[0], f"peakcheck-check{check_num:02}-lap{lap:02}-{frame:04}-all.png")
    ...
    # plt.axis("off")
    # plt.gray()
    # save_path_vector = os.path.join(savepath[1], f"peakcheck-check{check_num:02}-lap{lap:02}-{frame:04}-pred.png")
    # plt.savefig(save_path_vector, bbox_inches = 'tight', pad_inches = 0)
    ...
    left, right = plt.xlim()
    up, low = plt.ylim()
    plt.scatter(cellp[:, 1], cellp[:, 0], c="red", marker="o", s=20, linewidths=0)
    plt.xlim(left, right)
    plt.ylim(up, low)
    save_path_vector = os.path.join(savepath[2], f"peakcheck-check{check_num:02}-lap{lap:02}-{frame:04}-plot.pdf")
...
def main(root_path, check_num, lap):
    ...
    with tqdm(total=len(files[0]), leave=False, position=0) as pbar0:
        for frame, (fn) in enumerate(zip(files[0], files[1])):
            ...
            if frame == 0:
                plot(predictmask, cellimage, peaks[:, 1:], check_num, lap, frame, paths[-3:])
            pass

    save_path_vector = os.path.join(paths[-1], f"peak.txt")
